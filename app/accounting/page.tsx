...
import {
  ...
  Plus, Trash2, ChevronLeft, ChevronRight as ChevronRightIcon, Wallet,
} from 'lucide-react'
import {
...
export default function AccountingPage() {
  ...
  const [newExp, setNewExp] = useState({
    project_name: '', category: '', vendor: '', description: '', amount: '', payment_method: 'cash',
  })

  const CATEGORIES = ['Labour', 'Materials', 'Equipment', 'Transport', 'Subcontractor', 'MEP', 'Finishing', 'Overheads', 'Other']
  const PAY_METHODS = ['Cash', 'Bank Transfer', 'Credit Card', 'Cheque']

  const fetchDailyExpenses = useCallback(async (date: string) => {
    setDailyLoading(true)
    try {
      const res  = await fetch(`/api/daily-expenses?date=${date}`)
      const data = await res.json()
      setDailyExpenses(data.expenses ?? [])
    } catch { setDailyExpenses([]) }
    finally { setDailyLoading(false) }
  }, [])

  useEffect(() => { fetchDailyExpenses(dailyDate) }, [dailyDate, fetchDailyExpenses])
  ...
  async function addExpense() {
    ...
    const proj = allProjects.find(p => p.name === newExp.project_name)
    try {
      await fetch('/api/daily-expenses', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          expense_date:   dailyDate,
          project_id:     proj?.id ?? null,
          project_name:   newExp.project_name,
      ...
      setNewExp({ project_name: '', category: '', vendor: '', description: '', amount: '', payment_method: 'cash' })
      setShowAddForm(false)
      fetchDailyExpenses(dailyDate)
    } finally { setAddingExpense(false) }
  }

  async function deleteExpense(id: string) {
    await fetch(`/api/daily-expenses?id=${id}`, { method: 'DELETE' })
    setDailyExpenses(prev => prev.filter(e => e.id !== id))
  }

  const dailyTotal = dailyExpenses.reduce((s: number, e: any) => s + Number(e.amount), 0)
  const byCategory = dailyExpenses.reduce((acc: Record<string, number>, e: any) => {
  ...
  return (
    <div className="p-4 sm:p-6 animate-fade-in">
      ...
      <div className="mb-8 bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        {/* Header */}
        <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-3 flex-wrap bg-gradient-to-r from-indigo-900 to-indigo-700">
          <Wallet className="w-4 h-4 text-indigo-300 flex-shrink-0" />
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-white">Daily Expenses</h3>
            <p className="text-xs text-indigo-300 mt-0.5">Record and track daily site & office expenses</p>
          </div>

          {/* Date navigator */}
          <div className="flex items-center gap-1 bg-indigo-800 rounded-lg p-1">
            <button onClick={() => shiftDay(-1)} className="p-1.5 rounded-md hover:bg-indigo-700 text-indigo-300 hover:text-white transition-colors">
            ...
            <input
              ...
              className="text-xs bg-transparent border-0 outline-none text-white px-1 w-28"
            />
            <button onClick={() => shiftDay(1)} className="p-1.5 rounded-md hover:bg-indigo-700 text-indigo-300 hover:text-white transition-colors">
          ...
          <button
            ...
            className="text-xs text-indigo-300 hover:text-white border border-indigo-600 rounded-lg px-3 py-1.5 transition-colors"
          >
            Today
          </button>
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-white text-indigo-700 hover:bg-indigo-50 text-xs font-bold px-3 py-1.5 rounded-lg transition-colors"
          >
        ...
        {showAddForm && (
          <div className="px-5 py-4 bg-indigo-50 border-b border-indigo-100">
            <p className="text-xs font-bold text-indigo-700 mb-3">New expense for {fmtDate(dailyDate)}</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              <select
                value={newExp.project_name}
                onChange={e => setNewExp(p => ({ ...p, project_name: e.target.value }))}
                className="px-3 py-2 border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-white"
              >
              ...
              <select
                value={newExp.payment_method}
                onChange={e => setNewExp(p => ({ ...p, payment_method: e.target.value }))}
                className="px-3 py-2 border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-white"
              >
                {PAY_METHODS.map(m => <option key={m} value={m.toLowerCase().replace(' ', '_')}>{m}</option>)}
              </select>
            </div>

            <div className="flex justify-end gap-2 mt-3">
              <button onClick={() => setShowAddForm(false)} className="px-4 py-2 text-xs text-slate-600 border border-slate-200 rounded-lg hover:bg-white transition-colors">
                Cancel
              </button>
              <button
                onClick={addExpense}
                disabled={addingExpense || !newExp.amount}
                className="flex items-center gap-1.5 px-4 py-2 bg-indigo-700 hover:bg-indigo-600 disabled:opacity-50 text-white text-xs font-semibold rounded-lg transition-colors"
              >
        ...
        {dailyExpenses.length > 0 && (
          <div className="px-5 py-3 border-b border-slate-100 flex flex-wrap gap-2 items-center bg-slate-50">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Today's breakdown:</span>
            {Object.entries(byCategory).sort((a, b) => b[1] - a[1]).map(([cat, amt]) => (
              <span key={cat} className="inline-flex items-center gap-1 text-[11px] font-medium bg-white border border-slate-200 rounded-full px-2.5 py-1">
                <span className="text-slate-500">{cat}</span>
                <span className="font-bold text-slate-900">AED {Math.round(amt as number).toLocaleString()}</span>
              </span>
            ))}
            <span className="ml-auto text-sm font-bold text-indigo-900">Total: AED {Math.round(dailyTotal).toLocaleString()}</span>
          </div>
        )}
        ...
        {dailyLoading ? (
          ...
          <div className="py-10 text-center">
            ...
            <p className="text-sm text-slate-400 font-medium">No expenses for {fmtDate(dailyDate)}</p>
            <p className="text-xs text-slate-300 mt-1">Click "Add Expense" to record today's spending</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-xs min-w-[640px]">
              <thead>
                <tr className="bg-slate-50 text-left">
                  ...
                  <th className="px-4 py-2.5 text-[10px] font-bold text-slate-400 uppercase tracking-wide">Category</th>
                  <th className="px-4 py-2.5 text-[10px] font-bold text-slate-400 uppercase tracking-wide">Vendor</th>
                  <th className="px-4 py-2.5 text-[10px] font-bold text-slate-400 uppercase tracking-wide">Description</th>
                  <th className="px-4 py-2.5 text-[10px] font-bold text-slate-400 uppercase tracking-wide">Method</th>
                  <th className="px-5 py-2.5 text-[10px] font-bold text-slate-400 uppercase tracking-wide text-right">Amount</th>
                  <th className="px-4 py-2.5"></th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {dailyExpenses.map((e: any, i: number) => (
                  <tr key={e.id} className={`hover:bg-slate-50 transition-colors ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'}`}>
                    <td className="px-5 py-3">
                      {e.project_name
                        ? <span className="inline-flex items-center gap-1 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded-full px-2 py-0.5 text-[10px] font-semibold">{e.project_name}</span>
                        : <span className="text-slate-300">—</span>}
                    </td>
                    <td className="px-4 py-3">
                      {e.category
                        ? <span className="inline-flex items-center bg-slate-100 text-slate-700 rounded px-2 py-0.5 font-medium">{e.category}</span>
                        : <span className="text-slate-300">—</span>}
                    </td>
                    <td className="px-4 py-3 font-medium text-slate-800">{e.vendor || <span className="text-slate-300">—</span>}</td>
                    <td className="px-4 py-3 text-slate-500 italic max-w-[200px]">
                      <span className="block truncate" title={e.description}>{e.description || '—'}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="capitalize text-slate-600">{(e.payment_method || 'cash').replace('_', ' ')}</span>
                    </td>
              ...
              <tfoot>
                <tr className="bg-indigo-50 border-t-2 border-indigo-200">
                  <td colSpan={5} className="px-5 py-3 font-bold text-indigo-700 text-xs">
                    Total for {fmtDate(dailyDate)} · {dailyExpenses.length} entr{dailyExpenses.length === 1 ? 'y' : 'ies'}
                  </td>
                  <td className="px-5 py-3 text-right font-bold text-indigo-900">
                    AED {Math.round(dailyTotal).toLocaleString()}
                  </td>
                  <td />
                </tr>
              </tfoot>
