...
// ── Default template items (from BOQ FOR NEW VILLA.xlsx) ─────────────────────
...
const DEFAULT_ITEMS: BOQItem[] = [
  ...
  // 14. FIXING FLOORING AND WALL TILING
...
export default function CompanyBOQPage() {
  ...
  // Load existing BOQ
  useEffect(() => {
    fetch(`/api/boq/company?project_id=${projectId}`)
      .then(r => r.json())
      .then(data => {
        if (data && data.items) {
          setHeader({
          ...
          // Merge saved values into template (in case template has new items)
          ...
          setItems(DEFAULT_ITEMS.map(template => {
            const saved = savedMap.get(template.item_code)
            return saved ? { ...template, qty: saved.qty, rate: saved.rate, remarks: saved.remarks } : { ...template }
          }))
        }
      })
      .catch(() => {})
  ...
  const updateItem = useCallback((item_code: string, field: 'qty' | 'rate' | 'remarks', value: string | number) => {
    setItems(prev => prev.map(it =>
      it.item_code === item_code ? { ...it, [field]: field === 'remarks' ? value : Number(value) } : it
    ))
    setSaved(false)
  }, [])
    ...
    <div className="min-h-screen bg-slate-50 print:bg-white">
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-slate-200 px-4 sm:px-6 py-3 flex items-center justify-between gap-3 print:hidden">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-semibold text-slate-700">Company BOQ</span>
        </div>
      ...
      <div className="max-w-[1200px] mx-auto p-4 sm:p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3">
            {[
              ...
              <div key={field} className="flex items-center gap-2 border-b border-slate-100 pb-2">
                ...
                <input
                  className="flex-1 text-sm text-slate-900 font-medium bg-transparent outline-none focus:border-b focus:border-blue-400 placeholder:text-slate-300 print:pointer-events-none"
                  value={header[field]}
            ...
            <table className="w-full min-w-[900px] text-sm border-collapse">
              <thead>
                <tr className="bg-slate-800 text-white">
                  ...
                  <th className="px-3 py-2.5 text-left font-semibold border border-slate-600 w-20">ITEM</th>
                  ...
                  <th className="px-3 py-2.5 text-center font-semibold border border-slate-600 w-20">QTY</th>
                  <th className="px-3 py-2.5 text-center font-semibold border border-slate-600 w-24">RATE</th>
                  <th className="px-3 py-2.5 text-right font-semibold border border-slate-600 w-28">SUB TOTAL</th>
                  <th className="px-3 py-2.5 text-right font-semibold border border-slate-600 w-28">TOTAL</th>
                  <th className="px-3 py-2.5 text-left font-semibold border border-slate-600 print:hidden">REMARKS</th>
                </tr>
              </thead>
              <tbody>
                {sections.map(([sectionNo, { name, items: sectionItems }]) => {
                  const sectionTotal = sectionItems.reduce((s, it) => s + it.qty * it.rate, 0)
                  return (
                    <>
                      {/* Section header row */}
                      <tr key={`sec-${sectionNo}`} className="bg-blue-50 border-t-2 border-blue-200">
                      ...
                      {sectionItems.map((item, idx) => {
                        ...
                        return (
                          <tr key={item.item_code} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                            <td className="px-3 py-1.5 border border-slate-100 text-slate-400" />
                            ...
                            <td className="px-1 py-1 border border-slate-100 text-center">
                              <input
                                ...
                                className="w-full text-center text-sm font-medium bg-blue-50/50 border border-blue-100 rounded px-1 py-0.5 focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400 print:bg-transparent print:border-0"
                              />
                            </td>
                            {/* RATE — editable */}
                            <td className="px-1 py-1 border border-slate-100 text-center">
                              <input
                                ...
                                value={item.rate || ''}
                                placeholder="0"
                                onChange={e => updateItem(item.item_code, 'rate', e.target.value)}
                                className="w-full text-center text-sm font-medium bg-blue-50/50 border border-blue-100 rounded px-1 py-0.5 focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400 print:bg-transparent print:border-0"
                              />
                            </td>
                            {/* SUB TOTAL — auto */}
                            <td className="px-3 py-1.5 border border-slate-100 text-right font-medium text-slate-700">
                              {fmt(subTotal)}
                            </td>
                            {/* TOTAL — section total on first item */}
                            <td className="px-3 py-1.5 border border-slate-100 text-right font-bold text-slate-900">
                              {idx === 0 && sectionTotal > 0 ? fmt(sectionTotal) : ''}
                            </td>
                            {/* REMARKS — editable */}
                            <td className="px-1 py-1 border border-slate-100 print:hidden">
                              <input
                                ...
                                value={item.remarks}
                                placeholder="Notes…"
                                onChange={e => updateItem(item.item_code, 'remarks', e.target.value)}
                                className="w-full text-xs bg-transparent outline-none focus:border-b focus:border-slate-300 placeholder:text-slate-300"
                              />
                            </td>
                          </tr>
                        )
                      ...
                      <tr key={`total-${sectionNo}`} className="bg-slate-100">
                        <td colSpan={6} className="px-3 py-1.5 text-right text-xs font-semibold text-slate-600 border border-slate-200">
                          {name} — SECTION TOTAL
                        </td>
        ...
        {/* Summary Schedule */}
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden mt-6 print:shadow-none print:border-slate-400">
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-slate-100">
                ...
                <th className="px-4 py-2 text-right font-semibold text-slate-600 border border-slate-200 w-40">TOTAL AMOUNT (AED)</th>
              </tr>
            </thead>
            <tbody>
              {sections.map(([sectionNo, { name, items: sectionItems }]) => {
                const sectionTotal = sectionItems.reduce((s, it) => s + it.qty * it.rate, 0)
                return (
                  <tr key={sectionNo} className={sectionNo % 2 === 0 ? 'bg-slate-50/50' : 'bg-white'}>
                    <td className="px-4 py-1.5 border border-slate-100 font-medium text-slate-700">{sectionNo}</td>
                    <td className="px-4 py-1.5 border border-slate-100 text-slate-700">{name}</td>
                    <td className="px-4 py-1.5 border border-slate-100 text-right font-semibold text-slate-900">
                      {sectionTotal > 0 ? fmt(sectionTotal) : '—'}
                    </td>
                  </tr>
                )
              })}
          ...
          <div className="grid grid-cols-2 gap-12 mt-8">
            <div className="text-center">
              <div className="border-t border-slate-400 pt-2 mt-8">
                ...
                <p className="text-xs text-slate-500 mt-0.5">{header.owner || '___________________________'}</p>
              </div>
            </div>
            <div className="text-center">
              <div className="border-t border-slate-400 pt-2 mt-8">
                <p className="text-sm font-semibold text-slate-700">CONTRACTOR</p>
                <p className="text-xs text-slate-500 mt-0.5">{header.contractor}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <p className="text-center text-xs text-slate-400 mt-4 print:hidden">
          Highlight cells in blue to edit quantities and rates · Changes auto-save when you click Save BOQ
        </p>
      </div>
    </div>
  )
