...
// ── Default template (from Bill_of_Quantity (2) - Copy.xls) ──────────────────
...
const DEFAULT_ITEMS: MBHREItem[] = [
  ...
  // 17.0 – Cleaning, leveling & handing over
...
function getSectionLabel(srNo: string) {
  return srNo.endsWith('.0') ? parseInt(srNo) : null
}
...
export default function MBHREBOQPage() {
  ...
  // Load existing BOQ
  useEffect(() => {
    fetch(`/api/boq/mbhre?project_id=${projectId}`)
      .then(r => r.json())
      .then(data => {
        if (data && data.items) {
          setHeader({
          ...
          setItems(DEFAULT_ITEMS.map(template => {
            const sv = savedMap.get(template.sr_no)
            return sv ? { ...template, qty: sv.qty, rate: sv.rate } : { ...template }
          }))
        }
      })
      .catch(() => {})
  ...
  const updateItem = useCallback((sr_no: string, field: 'qty' | 'rate', value: string) => {
    setItems(prev => prev.map(it =>
      it.sr_no === sr_no ? { ...it, [field]: Number(value) } : it
    ))
    setSaved(false)
  }, [])
  ...
  async function handleSave() {
    setSaving(true)
    try {
      const res = await fetch('/api/boq/mbhre', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ project_id: projectId, ...header, items }),
      })
  ...
  // Group by section for section totals
  const sectionTotals = new Map<string, number>()
  let currentSection = ''
  items.forEach(it => {
    if (it.is_header) { currentSection = it.sr_no; sectionTotals.set(currentSection, 0) }
    else { sectionTotals.set(currentSection, (sectionTotals.get(currentSection) ?? 0) + it.qty * it.rate) }
  })
    ...
    <div className="min-h-screen bg-slate-50 print:bg-white" dir="ltr">
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-slate-200 px-4 sm:px-6 py-3 flex items-center justify-between gap-3 print:hidden">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-semibold text-slate-700">MBHRE BOQ — جدول الكميات</span>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => window.print()}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-slate-600 hover:text-slate-800 border border-slate-200 rounded-lg transition-colors"
          >
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="inline-flex items-center gap-1.5 px-4 py-1.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white text-sm font-semibold rounded-lg transition-colors"
          >
      ...
      <div className="max-w-[1100px] mx-auto p-4 sm:p-6">
        ...
        {/* Header Card — bilingual */}
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 mb-6 print:shadow-none print:border-slate-400">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-3" dir="rtl">
              {[
                ...
                <div key={field} className="flex items-center gap-2 border-b border-slate-100 pb-1.5">
                  ...
                  <input
                    className="flex-1 text-sm text-slate-900 bg-transparent outline-none focus:border-b focus:border-blue-400 placeholder:text-slate-300 text-right"
                    value={header[field]}
            ...
            <div className="space-y-3">
              ...
              ].map(({ en, field, placeholder }) => (
                <div key={field} className="flex items-center gap-2 border-b border-slate-100 pb-1.5">
                  <span className="text-xs font-bold text-slate-500 w-28 flex-shrink-0">{en}</span>
                  <input
                    className="flex-1 text-sm text-slate-900 bg-transparent outline-none focus:border-b focus:border-blue-400 placeholder:text-slate-300"
                    value={header[field]}
                    onChange={e => { setHeader(h => ({ ...h, [field]: e.target.value })); setSaved(false) }}
                    placeholder={placeholder}
                  />
        ...
        {/* BOQ Table */}
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden print:shadow-none print:border-slate-400">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[820px] text-sm border-collapse">
              <thead>
                {/* Arabic header row */}
                <tr className="bg-slate-700 text-white" dir="rtl">
                  <th className="px-3 py-2 text-center font-semibold border border-slate-600 w-16">الرقم</th>
                  <th className="px-3 py-2 text-right font-semibold border border-slate-600">وصف العمل (عربي)</th>
                  <th className="px-3 py-2 text-center font-semibold border border-slate-600 w-16">الكمية</th>
                  <th className="px-3 py-2 text-center font-semibold border border-slate-600 w-14">الوحدة</th>
                  <th className="px-3 py-2 text-center font-semibold border border-slate-600 w-22">السعر</th>
                  <th className="px-3 py-2 text-left font-semibold border border-slate-600">Description (English)</th>
                  <th className="px-3 py-2 text-right font-semibold border border-slate-600 w-28">القيمة / Amount</th>
                </tr>
                {/* English header row */}
                    ...
                    if (item.is_header) {
                      ...
                      return (
                        <tr key={item.sr_no} className="bg-blue-50 border-t-2 border-blue-300">
                          ...
                          <td className="px-3 py-2 font-bold text-blue-800 border border-blue-200 text-right" dir="rtl">
                            {item.arabic_desc}
                          </td>
                          <td colSpan={3} className="border border-blue-200" />
                          <td className="px-3 py-2 font-bold text-blue-800 border border-blue-200">
                            {item.english_desc}
                          </td>
                          <td className="px-3 py-2 font-bold text-blue-900 border border-blue-200 text-right">
                      ...
                      <tr key={item.sr_no} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                        ...
                        <td className="px-1 py-1 border border-slate-100 text-center">
                          <input
                            ...
                            value={item.qty || ''}
                            placeholder="0"
                            onChange={e => updateItem(item.sr_no, 'qty', e.target.value)}
                            className="w-full text-center text-sm font-medium bg-blue-50/50 border border-blue-100 rounded px-1 py-0.5 focus:outline-none focus:ring-1 focus:ring-blue-400 print:bg-transparent print:border-0"
                          />
                        </td>
                        ...
                        <td className="px-1 py-1 border border-slate-100 text-center">
                          <input
                            ...
                            value={item.rate || ''}
                            placeholder="0"
                            onChange={e => updateItem(item.sr_no, 'rate', e.target.value)}
                            className="w-full text-center text-sm font-medium bg-blue-50/50 border border-blue-100 rounded px-1 py-0.5 focus:outline-none focus:ring-1 focus:ring-blue-400 print:bg-transparent print:border-0"
                          />
                        </td>
                        <td className="px-3 py-1.5 border border-slate-100 text-slate-700">{item.english_desc}</td>
                        <td className="px-3 py-1.5 border border-slate-100 text-right font-semibold text-slate-900">
                          {amount > 0 ? fmt(amount) : ''}
                        </td>
                      </tr>
                    )
        ...
        {/* Signature Section */}
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 mt-6 print:shadow-none print:border-slate-400">
          <div className="grid grid-cols-3 gap-8 mt-10">
            <div className="text-center">
              <div className="border-t border-slate-400 pt-2 mt-8">
                <p className="text-sm font-bold text-slate-700">توقيع المالك</p>
                <p className="text-xs text-slate-500">Owner's Signature</p>
                <p className="text-xs text-slate-400 mt-1">{header.owner_name || '___________________________'}</p>
              </div>
            </div>
            <div className="text-center">
              <div className="border-t border-slate-400 pt-2 mt-8">
                <p className="text-sm font-bold text-slate-700">توقيع وختم المقاول</p>
                <p className="text-xs text-slate-500">Contractor's Stamp & Signature</p>
                <p className="text-xs text-slate-400 mt-1">{header.contractor}</p>
              </div>
            </div>
            <div className="text-center">
              <div className="border-t border-slate-400 pt-2 mt-8">
                <p className="text-sm font-bold text-slate-700">توقيع وختم الاستشاري</p>
                <p className="text-xs text-slate-500">Consultant's Stamp & Signature</p>
                <p className="text-xs text-slate-400 mt-1">{header.consultant || '___________________________'}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <p className="text-center text-xs text-slate-400 mt-4 print:hidden">
          Blue cells are editable — fill in Qty and Rate · Amount calculates automatically · Click Save BOQ to store
        </p>
      </div>
    </div>
  )
