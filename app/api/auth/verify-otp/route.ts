...
export async function POST(req: Request) {
  try {
    ...
    // Read OTP from signed cookie (stateless — works across Vercel instances)
    const otpCookieValue = req.headers.get('cookie')
      ?.split(';')
      .find(c => c.trim().startsWith('sama-otp='))
      ?.split('=').slice(1).join('=')
      .trim()
    ...
    if (code !== otp.code) {
      ...
      const updatedCookie = bumpAttempts(otpCookieValue)
      const res = NextResponse.json(
        { error: `Incorrect code. ${left} attempt${left === 1 ? '' : 's'} remaining.` },
        { status: 401 }
      )
      res.cookies.set('sama-otp', updatedCookie, {
