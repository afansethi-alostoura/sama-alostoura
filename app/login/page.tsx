...
import { AlertCircle, Loader2, Lock, MessageCircle, RefreshCw } from 'lucide-react'
...
export default function LoginPage() {
  ...
  async function handleCredentials(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.step === 'done') {
        // WhatsApp not configured or unavailable — logged in directly
        router.push('/')
        return
      }
  ...
  useEffect(() => {
    if (step === 'otp' && otp.replace(/\s/g, '').length === 6 && !expired && !loading) {
      handleVerify()
    }
  }, [otp])
          ...
          {step === 'credentials' && (
            <form onSubmit={handleCredentials} className="px-8 py-8 space-y-5">
              ...
              <p className="text-center text-xs text-slate-400 flex items-center justify-center gap-1.5">
                ...
                A WhatsApp verification code will be sent
              </p>
            </form>
          )}
          ...
          {step === 'otp' && (
            <div className="px-8 py-8 space-y-6">
              ...
              <div className="text-center text-sm text-slate-500">
                {!expired
                  ? <>Expires in <Countdown seconds={600} onExpire={() => setExpired(true)} /></>
              ...
              <button onClick={() => { setStep('credentials'); setOtp(''); setError(''); setExpired(false) }}
